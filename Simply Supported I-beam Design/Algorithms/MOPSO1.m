...
%     VEL = zeros(Np,nVar);
% %     POS_fit  = fun(POS);
% a.s=unifrnd(var_min,var_max,nVar);
%     POS = repmat((var_max-var_min)',Np,1).*rand(Np,nVar) + repmat(var_min',Np,1);
%     
%     VEL=zeros(nVar,Np);
for i=1:Np
...
%         display(['Generation #' num2str(gen) ' - Repository size: ' num2str(size(REP.pos,1))]);
     ...
     rst.ppareto{gen}=POS'; %设计变量
     rst.fpareto{gen}=POS_fit; %目标函数
     rst.gpareto{gen}=gi; %约束条件,目前要修改
...
function POS = mutation(POS,gen,maxgen,Np,var_max,var_min,nVar,u_mut)
    ...
    if(nmut>0)
        ...
        a1=(repmat((var_max-var_min)',nmut,1).*rand(nmut*nVar,1) + repmat(var_min',nmut,1))';
        for i=1:length(idx)
        POS(idx(i),:) = a1(1,1+(i-1)*nVar:nVar+(i-1)*nVar);
        end
    end
